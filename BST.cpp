...
{  // CD1:Si node->data==d , ¿no se insertan duplicados? <<--
    ...
    else if (node->data.getPar()>d.getPar()) {// d es mes petita que la node->dada (el valor que apunta el punter node)
        node->left = insert(node->left, d);
    } 
    else if (node->data.getPar()<d.getPar()) {//d es mes gran que la node->dada (el valor que apunta el punter node)
